...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDefaultMemberPermissions(PermissionFlagsBits[config.commands.unclaim.permission])
    ...
    async execute(interaction) {
        ...
        if (await ticketsDB.get(`${interaction.channel.id}.claimed`) === false) {
            return interaction.reply({ content: "This ticket has not been claimed!", ephemeral: true });
          }

        if (await ticketsDB.get(`${interaction.channel.id}.claimUser`) !== interaction.user.id) {
            return interaction.reply({ content: `You did not claim this ticket, only the user that claimed this ticket can unclaim it! (<@!${await ticketsDB.get(`${interaction.channel.id}.claimUser`)}>)`, ephemeral: true  });
          }
