...
const { ticketsDB, sanitizeInput, logMessage, ticketCategories } = require('../../index.js');
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName('category').setDescription('Input a Category Name').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits[config.commands.move.permission])
    ...
    async execute(interaction) {
        ...
        if (!choices.includes(option)) {
            return interaction.reply({ content: `Invalid option. Available options are: ${choices.join(', ')}`, ephemeral: true });
        }
        
        if (option === ticketType) {
            return interaction.reply({ content: "This ticket is already in that category.", ephemeral: true });
        }
        ...
        .addFields([
            { name: config.commands.move.LogEmbed.field_staff, value: `> ${interaction.user}\n> ${sanitizeInput(interaction.user.tag)}` },
            { name: config.commands.move.LogEmbed.field_ticket, value: `> ${interaction.channel}\n> #${sanitizeInput(interaction.channel.name)}` },
         ])
        .setTimestamp()
        ...
        .setDescription(`${config.commands.move.embed.description}`.replace(/\{category\}/g, option))
        interaction.reply({ embeds: [embed] });
        logMessage(`${interaction.user.tag} moved the ticket #${interaction.channel.name} to the category ${option}.`);
