...
const { ticketsDB, sanitizeInput, logMessage, client } = require('../../index.js');
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDefaultMemberPermissions(PermissionFlagsBits[config.commands.transfer.permission])
    ...
    async execute(interaction) {
        ...
        let currentUser = client.users.cache.get(await ticketsDB.get(`${interaction.channel.id}.userID`));
        
        if (optionUser === currentUser) {
            return interaction.reply({ content: "This user is already the creator of this ticket.", ephemeral: true });
        }
        ...
        .addFields([
            { name: config.commands.transfer.LogEmbed.field_staff, value: `> ${interaction.user}\n> ${sanitizeInput(interaction.user.tag)}` },
            { name: config.commands.transfer.LogEmbed.field_ticket, value: `> ${interaction.channel}\n> #${sanitizeInput(interaction.channel.name)}` },
            { name: config.commands.transfer.LogEmbed.field_transfer, value: `> ${currentUser} (${sanitizeInput(currentUser.tag)}) -> ${optionUser} (${sanitizeInput(optionUser.tag)})` },
         ])
        .setTimestamp()
        ...
        .setDescription(`${config.commands.transfer.embed.description}`.replace(/\{user\}/g, optionUser).replace(/\{user\.tag\}/g, sanitizeInput(optionUser.tag)))
        interaction.reply({ embeds: [embed] });
        logMessage(`${interaction.user.tag} transferred the ownership of the ticket #${interaction.channel.name} to the user ${optionUser.tag}.`);
