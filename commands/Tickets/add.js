...
module.exports = {
    ...
    async execute(interaction) {
        ...
        .addFields([
            ...
            { name: config.commands.add.LogEmbed.field_target, value: `> ${user}\n> ${sanitizeInput(user.tag)}` },
        ...
        .setDescription(`${config.commands.add.embed.description}`.replace(/\{target\}/g, user).replace(/\{target\.tag\}/g, sanitizeInput(user.tag)))
        ...
        if (role) {
            ...
            .addFields([
                { name: config.commands.add.LogEmbed.field_staff, value: `> ${interaction.user}\n> ${sanitizeInput(interaction.user.tag)}` },
                { name: config.commands.add.LogEmbed.field_target, value: `> ${role}\n> ${sanitizeInput(role.name)}` },
                { name: config.commands.add.LogEmbed.field_ticket, value: `> ${interaction.channel}\n> #${sanitizeInput(interaction.channel.name)}` },
             ])
            .setTimestamp()
            .setThumbnail(interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }))
            ...
            const embed = new EmbedBuilder()
            .setColor(config.commands.add.embed.color)
            .setDescription(`${config.commands.add.embed.description}`.replace(/\{target\}/g, role).replace(/\{target\.tag\}/g, sanitizeInput(role.name)))
            interaction.reply({ embeds: [embed] });
            logsChannel.send({ embeds: [logEmbed] });
            logMessage(`${interaction.user.tag} added ${role.name} to the ticket #${interaction.channel.name}`);
            }
